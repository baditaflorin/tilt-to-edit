...
declare global {
  interface ScriptTagEmbedSnapshot {
    intentVector: {
      x: number;
      y: number;
    };
  ...
  interface ScriptTagEmbedDetail {
    snapshot: ScriptTagEmbedSnapshot;
    selectedIndex?: number | undefined;
  }

  interface ScriptTagEmbedInstance {
    requestPermission: () => Promise<unknown>;
    calibrate: () => boolean;
  }

  interface ScriptTagEmbedGlobal {
    getInstances: () => ScriptTagEmbedInstance[];
    scan: (root?: ParentNode | Document) => ScriptTagEmbedInstance[];
  }

  interface Window {
    TiltToEdit?: ScriptTagEmbedGlobal;
  }
}
...
const BACKDROPS: readonly [BackdropOption, ...BackdropOption[]] = [
  {
    name: "Neon Alley",
...
const CHARACTERS: readonly [CharacterOption, ...CharacterOption[]] = [
  {
    name: "Pilot",
...
const scenePreview = requireElement<HTMLDivElement>("scene-preview");
const sceneGlow = requireElement<HTMLDivElement>("scene-glow");
const sceneSkyline = requireElement<HTMLDivElement>("scene-skyline");
const sceneGround = requireElement<HTMLDivElement>("scene-ground");
const sceneBeam = requireElement<HTMLDivElement>("scene-beam");
const worldName = requireElement<HTMLElement>("world-name");
const worldSummary = requireElement<HTMLParagraphElement>("world-summary");
const characterName = requireElement<HTMLElement>("character-name");
const characterSummary = requireElement<HTMLParagraphElement>("character-summary");
const characterAura = requireElement<HTMLDivElement>("character-aura");
const characterShadow = requireElement<HTMLDivElement>("character-shadow");
const characterHead = requireElement<HTMLDivElement>("character-head");
const characterBody = requireElement<HTMLDivElement>("character-body");
const characterCore = requireElement<HTMLDivElement>("character-core");
const characterRoot = requireElement<HTMLDivElement>("scene-character");
...
function renderScene() {
  ...
  scenePreview.setAttribute(
    "style",
    `background: ${backdrop.background}; box-shadow: 0 32px 72px rgba(8, 15, 28, 0.44);`,
  );
  sceneGlow.setAttribute("style", `background: ${backdrop.glow};`);
  sceneSkyline.setAttribute("style", `color: ${backdrop.skyline};`);
  sceneGround.setAttribute(
    "style",
    `background: linear-gradient(180deg, ${backdrop.surface}, rgba(255,255,255,0.02));`,
  );
  sceneBeam.setAttribute(
    "style",
    `background: linear-gradient(90deg, rgba(255,255,255,0.04), ${backdrop.accent}); box-shadow: 0 0 24px ${backdrop.accent};`,
  );
  ...
  characterAura.setAttribute(
    "style",
    `background: radial-gradient(circle, ${character.aura} 0%, transparent 68%);`,
  );
  ...
  characterBody.setAttribute(
    "style",
    `background: linear-gradient(180deg, ${character.coat}, ${character.core});`,
  );
  characterCore.setAttribute(
    "style",
    `background: ${character.core}; box-shadow: 0 0 18px ${character.core}, 0 0 34px ${character.aura};`,
  );
}
...
sceneDriver.addEventListener("tilt-to-edit:state", (event) => {
  const detail = event as CustomEvent<ScriptTagEmbedDetail>;
